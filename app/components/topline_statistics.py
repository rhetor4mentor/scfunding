...
def get_topline_statistics():
    ...
    chart_pledge_recent = charts.plot_current_vs_last_year(ts_daily, metric='pledges', show_title=False, cap_last_x_days=CAP_LAST_DAYS).properties(height=250)
    chart_citizens_recent = charts.plot_current_vs_last_year(ts_daily, metric='citizens', show_title=False, cap_last_x_days=CAP_LAST_DAYS).properties(height=250)
    ...
    with top_left:
        ...
        with top_left_right:
            st.altair_chart(chart_pledge_recent, use_container_width=True, theme="streamlit",)
    ...
    with top_middle:
        ...
        with middle_left:
            ...
            st.metric(label=f"Accounts registered in {year}", 
        ...
        with middle_right:
            st.altair_chart(chart_citizens_recent, use_container_width=True, theme="streamlit")

    with top_right:
        st.metric(label="Last data update", 
