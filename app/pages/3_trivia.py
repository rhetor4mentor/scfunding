...
def trivia():
    ...
    data_citizens = observations.precedence(
        df=st.session_state["ts_daily"],
        timestamp=pd.Timestamp(date_input),
        metric="delta_citizens",
    )
    data_pledges = observations.precedence(
        df=st.session_state["ts_daily"],
        timestamp=pd.Timestamp(date_input),
        metric="delta_pledge",
    )
    ...
    with middle_right:
        ...
        if selected_year:
            year_view = st.session_state["cts"].game_version_parser.year_view(
                selected_year
            )
