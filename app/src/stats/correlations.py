...
def compute_correlations_with_sliding_window(
    ...
    logger.info(f"correlations - computing correlations of {x} with {y} for {len(periods)} periods of size {window_size} (time resolution: {freq})")
    
    for start in tqdm(periods):
        end = start + window_size
        window_df = df.iloc[start:end]
        
        corr = pg.corr(window_df[x], window_df[y], method='percbend',)
