...
def records(
    df: pd.DataFrame, metric: str = "delta_pledge", n=5, ascending=False
) -> pd.DataFrame:
    """
    ...
    columns_order = priority_cols + [
        col for col in sorted_df.columns if col not in priority_cols
    ]
...
def precedence(
    df: pd.DataFrame, timestamp: pd.Timestamp = None, metric: str = "delta_pledge"
) -> pd.DataFrame:
    """
    ...
    rows_prior_with_higher_or_equal_metric = prior_rows[
        prior_rows[metric] >= metric_value_at_timestamp
    ]
    percentage_prior = (
        (len(rows_prior_with_higher_or_equal_metric) / len(prior_rows))
        if len(prior_rows) > 0
        else 0
    )
