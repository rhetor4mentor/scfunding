...
class HourlyTransactions(BaseModel):
    ...
    @validator(
        "total_citizens", "delta_citizens", "data_correction_total_citizen", pre=True
    )
    ...
    def check_pledge_and_citizens(cls, values):
        if not (
            values.get("total_pledge") or values.get("data_correction_total_pledge")
        ):
            raise logger.error(
                "Either total_pledge or data_correction_total_pledge must be provided."
            )
        if not (
            values.get("total_citizens") or values.get("data_correction_total_citizen")
        ):
            raise logger.error(
                "Either total_citizens or data_correction_total_citizen must be provided."
            )
        return values
    ...
    def data(self) -> ValidatedTransactionData:
        ...
        total_pledge = (
            self.data_correction_total_pledge
            if self.data_correction_total_pledge is not None
            else self.total_pledge
        )
        total_citizens = (
            self.data_correction_total_citizen
            if self.data_correction_total_citizen is not None
            else self.total_citizens
        )
