...
class CompleteTimeSeries:
    ...
    def get_time_series(self, freq: str = "D") -> pd.DataFrame:
        transactions = self.transaction_parser.get_time_series(
            freq=freq, append_time_metrics=False
        )
        calendar_events = self.calendar_event_parser.get_time_series(
            freq=freq, append_time_metrics=False
        )
        game_versions = self.game_version_parser.get_time_series_enriched(
            freq=freq, append_time_metrics=False
        )
        ...
        combined_df = self.transaction_parser.time_series_constructor.add_time_metrics(
        ...
        if output_max_date > max_date:
            logger.warning(
                f"max date introduced through ts combination: {output_max_date} vs expected {max_date}"
            )
    ...
    def patch_stats(
        ...
        """
        first_alpha01_patch_date = self.game_version_parser.versions.query(
            "major==1"
        ).iloc[0]["date_start"]

        patch_stats = (
            self.time_series[self.time_series.index >= first_alpha01_patch_date]
            .reset_index()
            .groupby(["version_id"])
            .agg(
                {
