...
class TimeSeriesConstructor:
    ...
    def __init__(
        ...
        if self.aggregation_functions is None:
            logger.warning(
                f"TimeSeriesConstructor instance initiated without aggregation_functions, will use a harcoded default"
            )
    ...
    def add_time_metrics(self, time_series: pd.DataFrame) -> pd.DataFrame:
        ...
        if freq_numeric <= 4:
            ...
            time_series["is_weekend"] = time_series["day_of_week"].apply(
        ...
        time_series["period"] = (
            pd.Series(time_series.index).apply(utils.format_timestamp, freq=freq).values
        )
    ...
    def recompute_summables(self, time_series: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        if (not "delta_citizens" in time_series.columns) and (
            "delta_citizens" in self.summables
        ):
            time_series.loc[:, "delta_citizens"] = time_series["total_citizens"].diff()
            time_series["delta_citizens"].iloc[0] = time_series["total_citizens"].iloc[
                0
            ]
        if (not "delta_pledge" in time_series.columns) and (
            "delta_pledge" in self.summables
        ):
    ...
    def add_totals(self, time_series: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        if (not "total_citizens" in time_series.columns) and (
            "delta_citizens" in self.summables
        ):
            time_series.loc[:, "total_citizens"] = time_series[
                "delta_citizens"
            ].cumsum()
        if (not "total_pledge" in time_series.columns) and (
            "delta_citizens" in self.summables
        ):
    ...
    def add_rolling_totals(
        self, time_series: pd.DataFrame, last_periods: int = 30
    ) -> pd.DataFrame:
        """
        ...
        for summable in self.summables:
            ...
            time_series[column_name] = (
                time_series[summable]
                .rolling(window=last_periods, min_periods=1)
                .sum()
                .shift(1)
            )
    ...
    def add_averages(self, time_series: pd.DataFrame) -> pd.DataFrame:
        ...
        if ("total_citizens" in time_series.columns) and (
            "total_pledge" in time_series.columns
        ):
    ...
    def add_in_year_totals(self, time_series: pd.DataFrame) -> pd.DataFrame:
        if "delta_citizens" in self.summables:
            time_series["total_citizens_in_year"] = time_series.groupby(
                time_series.index.year
            )["delta_citizens"].cumsum()

        if "delta_pledge" in self.summables:
            time_series["total_pledge_in_year"] = time_series.groupby(
                time_series.index.year
            )["delta_pledge"].cumsum()
    ...
    def process(
        ...
        df = (
            ...
            .pipe(self.recompute_summables)
            .pipe(self.add_in_year_totals)
        )
        self.deepest_time_series = df

    def get(
        self, freq: str = "D", last_periods: int = 30, append_time_metrics: bool = True
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"Providing time series at desired frequency ({freq}) for caller: {caller_name}"
        )

        time_series = (
            self.deepest_time_series[list(self.aggregation_functions.keys())]
            .resample(
                freq,
            )
        ...
        output = (
            time_series.pipe(self.add_totals)
