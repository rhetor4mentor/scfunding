...
class FileLoader:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.raw_data = pd.read_csv(address, usecols=lambda column: not column.startswith('Unnamed'), low_memory=False)
    ...
    def get_time_series(self,
        ...
        try:
            time_series = self.time_series_constructor.get(freq, append_time_metrics=append_time_metrics)
            return time_series
        except Exception as e:
            logger.error(e)
            return pd.DataFrame() 
...
class TransactionParser(FileLoader):
    ...
    def __init__(self, file_path: str = None):
        ...
        self.time_series_constructor = TimeSeriesConstructor(transactions, aggregation_functions=self.aggregation_functions)
    ...
    def parse_transactions(self, dataframe: pd.DataFrame) -> List[HourlyTransactions]:
        ...
        column_map: dict = dict(zip(dataframe.columns, [utils.format_to_column(c) for c in dataframe.columns]))
    ...
    def parse_dataframe(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(unsuccessful_records) > 0:
            logger.warning(f"Excluded {len(unsuccessful_records)} records that did not conform to ValidatedTransactionData requirements")
        ...
        suspicious_dates = dataframe['datetime_utc'][dataframe['datetime_utc'].diff() < pd.Timedelta(days=0) ]

        if len(suspicious_dates) % 2 != 0:
            logger.warning("I probably did not solve corrupted dates")

        suspicious_indices = suspicious_dates.shift(-1).dt.month == suspicious_dates.dt.day
        for i in range(0, len( suspicious_indices), 2):
            if i + 1 < len(suspicious_indices):
                if suspicious_indices.iloc[i] and not suspicious_indices.iloc[i+1]:
                    ...
                    corrected_dates = dataframe.loc[start_idx:end_idx]['datetime_utc'].apply(self.correct_datetime_format)
                    dataframe.loc[start_idx:end_idx, 'datetime_utc'] = corrected_dates

        duplicated_timestamps = dataframe['datetime_utc'][dataframe['datetime_utc'].duplicated(keep='first')]
        ...
        if len(duplicated_timestamps) > 0:
            ...
            for duplication_event in problematic_dates:
                ...
                matching_date_indexes = dataframe.index[dataframe['datetime_utc'].dt.date == duplication_event].to_series()
                point_of_rupture =  matching_date_indexes[matching_date_indexes.diff() >= 24].values
                ...
                if len(point_of_rupture) > 1:
                    logger.error(f"more than one point-of-rupture defined when trying to correct corrupted dates for {duplication_event.date()}")
                elif len(point_of_rupture) == 0:
                    logger.error(f"no point-of-rupture defined when trying to correct corrupted dates for {duplication_event.date()}")
                else:
                    first_island = matching_date_indexes.loc[:point_of_rupture[0]-1]
                    second_island = matching_date_indexes.loc[point_of_rupture[0]:]

                    if len(first_island) + len(second_island) != len(matching_date_indexes):
                        logger.error(f"incorrect island segmentation for {duplication_event.date()}")
                    ...
                    corrected_dates = dataframe.iloc[incorrect_date_indexes]['datetime_utc'].apply(self.correct_datetime_format)
                    dataframe.loc[incorrect_date_indexes, 'datetime_utc'] = corrected_dates

            if len(set(dataframe['datetime_utc'])) == len(dataframe):
                logger.success(f"Corrected {len(problematic_dates)} dates recorded with reverted YYYY/DD/MM format.")
            else:
                logger.warning(f"Still duplicates: {dataframe['datetime_utc'][dataframe['datetime_utc'].duplicated(keep=False)]}")

        return dataframe.set_index('datetime_utc')


    @property
    def main_statistics(self,) -> dict:
        '''
        ...
        output['pledges']['pct_change_year_on_year'] = output['pledges']['total_this_year']/output['pledges']['total_year_on_year'] - 1 
        output['citizens']['pct_change_year_on_year'] = output['citizens']['total_this_year']/output['citizens']['total_year_on_year'] - 1 
...
class CalendarEventParser(FileLoader):
    ...
    def parse(self, dataframe: pd.DataFrame = None) -> pd.DataFrame:
        ...
        valid_df = pd.DataFrame([record.dict() for record in valid_records]).set_index('datetime_utc')
...
class GameVersionParser(FileLoader):
    ...
    def __init__(self, file_path: str = None):
        ...
        self.aggregation_functions = {
            'days_since_current_patch_launch': [('days_since_current_patch_launch', 'max')],
            'patch_count': [
    ...
    @property
    def version_patch_count_map(self,) -> dict:
        count_to_version_map = self.versions[['patch_count', 'version']].drop_duplicates().set_index('patch_count')['version'].to_dict()
    ...
    def parse(self, dataframe: pd.DataFrame = None) -> pd.DataFrame:
        ...
        df[['major', 'minor', 'patch']] = df['version'].apply(lambda v: pd.Series(utils.parse_version(v)))
    ...
    def parse_to_long(self, dataframe: pd.DataFrame) -> pd.DataFrame:
        expanded_df = pd.concat(dataframe.apply(self.expand_intervals, axis=1).tolist(), ignore_index=True)
    ...
    def get_time_series_enriched(self, freq: str = 'D', append_time_metrics: bool = True) -> pd.DataFrame:
        ts = self.get_time_series(freq, append_time_metrics)
        ts['version_id'] = ts['version_id'].replace(self.version_patch_count_map).astype(str)
        return ts
    ...
    def year_view(self, year: int = None) -> pd.DataFrame:
        ...
        data = self.versions_long[year_filter][['version', 'major', 'minor', 'patch']].drop_duplicates().sort_index().reset_index()
        data['date'] = data['datetime_utc']

        data['days_live'] = (pd.to_datetime(data['date'].shift(-1)) - pd.to_datetime((data['date']))).dt.days
        ...
        output = data[['version', 'date', 'days_live', 'pct_live', 'major', 'minor', 'patch']]   

        return output

if __name__ == '__main__':
    pd.set_option('display.max_rows', 1000)
    self = TransactionParser('test/data/Crowdfunding Development Spreadsheet Version 3.0 - Hourly Data Import.csv')
