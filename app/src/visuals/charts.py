...
def generate_tooltip(dataframe: pd.DataFrame) -> List[alt.Tooltip]:
    ...
    tooltips.extend(
        (
            alt.Tooltip(
                column, format=formats[column], title=utils.format_to_title(column)
            )
            if formats[column] is not None
            else alt.Tooltip(column, title=utils.format_to_title(column))
        )
        for column in formats.keys()
        if not column.startswith("period")
    )
...
def plot_line_chart(
    ...
    hover = alt.selection_point(
        on="mouseover", nearest=True, empty="none", fields=[index]
    )
    ...
    legend = (
        ...
        .properties(width="container", height=25)
    )

    chart = (
        alt.layer(*lines_and_points)
        .resolve_scale(
            y="independent",
            x="shared",
        )
...
def plot_all_years(
    ...
    if metric == "pledges":
        ...
        raise ValueError(
            f"Only values permitted are 'pledges' and 'citizens', not {metric}"
        )

    x_axis = alt.X(
        f"delta_{x}:Q",
        title=None,
        axis=alt.Axis(format=x_format, labelOverlap=True),
    ...
    subtitle = (
        f"{total:,.0f} " if metric != "pledges" else f"${total:,.0f} "
    ) + "historically"
...
def plot_transactions_years_to_date(
    ...
    x_axis = alt.X(
        ...
        axis=alt.Axis(
            ...
            labelExpr=(
                ...
                if metric == "pledges"
                else "format(datum.value / 1000, ',.0f') + 'K'"
            ),
        ),
        scale=alt.Scale(domainMin=0),
    )
    ...
    df[pct_metric] = np.where(
        latest_value != 0, (latest_value - df[x]) / latest_value, np.nan
    )
    ...
    bars = (
        ...
        .encode(
            ...
            color=alt.Color(
                "is_current:O",
                scale=alt.Scale(range=YEAR_COMPARISON_COLORS),
                title="",
                legend=None,
            ),
            tooltip=tooltips,
...
def plot_current_vs_last_year(
    ...
    df = ts.query(
        f"(year in [{year_filter-1}, {year_filter}]) & (day_of_year<={day_filter})"
    ).copy()
    ...
    df_tooltip = df[["day_of_year", "period", y, "year", "version_id"]].pivot(
        index="day_of_year", columns=["year"]
    )
    df_tooltip.rename(columns={"version_id": "version"}, inplace=True)
    df_tooltip.columns = [
        f"{col[0]}_{int(col[1])}" for col in df_tooltip.columns.values
    ]
    ...
    lines = (
        ...
        .encode(
            ...
            color=alt.Color(
                "year:O",
                sort=[year_filter, year_filter - 1],
                scale=alt.Scale(range=YEAR_COMPARISON_COLORS[::-1]),
                title="Year",
            ),
        )
    ...
    points = lines.mark_point().encode(
        x=x_axis, opacity=alt.condition(hover, alt.value(1), alt.value(0))
    )
