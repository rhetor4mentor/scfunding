...
def same_time_in_other_year(ts: pd.DataFrame, date: pd.Timestamp = None, year: int = None) -> pd.DataFrame:
    '''
    Arguments:
    ---------
    ...
    mask = (time_series.index.dayofyear == date.dayofyear) & (time_series.index.year == year)
    
    latest_row = time_series.loc[mask].iloc[-1] if not time_series.loc[mask].empty else None
